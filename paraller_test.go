...
func TestAsync1(t *testing.T) {
	...
	for _, s := range st {
		var m3 string
		err := Await(s.ctx, Async(func() error {
			if err := Await(s.ctx, Async(s.f1), Async(s.f2)); err != nil {
				return err
			}
		...
		if m3 != s.m3 {
			t.Errorf("m2 %v: expect %v", m3, s.m3)
		}
	}
